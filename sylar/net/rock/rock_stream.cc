...
// 读取一条消息并根据类型进行处理：
// - RESPONSE：匹配 sn 唤醒等待中的请求上下文
// - REQUEST/NOTIFY：派发到业务回调
...
// 创建 Rock 连接的回调，由 SDLoadBalance 使用
                                            ...
                                            ServiceItemInfo::ptr info)
{
    ...
    sylar::WorkerMgr::GetInstance()->schedule("service_io", [conn, addr, domain, service]() {
        if (domain == "logserver") { // 特殊域：建立连接后自动登录
