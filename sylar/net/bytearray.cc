...
ByteArray::ByteArray(size_t base_size)
    : m_baseSize(base_size), m_position(0), m_capacity(base_size), m_size(0),
      m_endian(SYLAR_BIG_ENDIAN), m_owner(true), m_root(new Node(base_size)), m_cur(m_root)
{
}

ByteArray::ByteArray(void *data, size_t size, bool owner)
    : m_baseSize(size), m_position(0), m_capacity(size), m_size(size), m_endian(SYLAR_BIG_ENDIAN),
      m_owner(owner)
{
    m_root = new Node();
