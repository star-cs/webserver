...
struct WorkerIniter{
    WorkerIniter(){
        g_worker_config->addListener([](const std::map<std::string, std::map<std::string, std::string> >& old_value,
                                      const std::map<std::string, std::map<std::string, std::string> >& new_value) {
            WorkerMgr::GetInstance()->init(new_value);
        });
    }
};

static WorkerIniter _workerIniter;
