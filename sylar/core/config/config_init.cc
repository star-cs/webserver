...
public:
    LogDefine operator()(const std::string &str)
    {
        ...
        if (node["buf_mgr"].IsDefined()) {
            ...
            // 必须全部定义好
            bmd.work_type = buf_mgr_node["work_type"].IsDefined()
                                ? buf_mgr_node["work_type"].as<std::string>()
                                : "";
            bmd.type = buf_mgr_node["type"].IsDefined()
                           ? AsyncType::FromString(buf_mgr_node["type"].as<std::string>())
                           : AsyncType::Type::UNKNOW;
            bmd.size = buf_mgr_node["size"].IsDefined() ? buf_mgr_node["size"].as<size_t>() : 0;
            bmd.threshold =
                buf_mgr_node["threshold"].IsDefined() ? buf_mgr_node["threshold"].as<size_t>() : 0;
            bmd.linear_growth = buf_mgr_node["linear_growth"].IsDefined()
                                    ? buf_mgr_node["linear_growth"].as<size_t>()
                                    : 0;
            bmd.swap_time =
                buf_mgr_node["swap_time"].IsDefined() ? buf_mgr_node["swap_time"].as<size_t>() : 0;
                ...
                switch (lad.type) {
                    ...
                    case AppenderType::Type::FileLogAppender:
                        ...
                        if (!it["file"].IsDefined()) {
                            std::cout << "log config error: FilelogAppender file is null, " << it
                                      << std::endl;
                            continue;
                        }
                        lad.file = it["file"].as<std::string>();
                        lad.flush_rule =
                            it["flush_rule"].IsDefined()
                                ? FlushRule::FromString(it["flush_rule"].as<std::string>())
                                : FlushRule::Rule::FFLUSH;
                        break;
                    case AppenderType::Type::RotatingFileLogAppender:
                        ...
                        if (!it["max_size"].IsDefined()) {
                            std::cout
                                << "log config error: RotatingFileLogAppender max_size is null, "
                                << it << std::endl;
                        }
                        lad.max_size = it["max_size"].as<size_t>();
                        lad.max_file = it["max_file"].IsDefined() ? it["max_file"].as<size_t>()
                                                                  : 0; // 0，就是无限增加
                        break;
                    default:
                        std::cout << "log config error: appender type is invalid, " << it
                                  << std::endl;
                }
...
static ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    Config::Lookup("logs", std::set<LogDefine>(), "logs");
...
struct LogIniter {
    ...
    { // 构造函数
         ...
         */
        g_log_defines->addListener([](const std::set<LogDefine> &old_log,
                                      const std::set<LogDefine> &new_log) {
            // std::cout << "测试 仿函数调用" << std::endl;
            // 新增
            // 修改
            // 删除
            for (auto &i : new_log) {
                auto it = old_log.find(
                    i); // set<T> 按照 operator< 查找，那么也就是按照LogDefine name排序及查找
                ...
                for (auto &a : i.appenders) {
                    ...
                    if (!a.pattern.empty()) {
                        ...
                        if (lap->isError()) { // 如果日志格式有错误
                            std::cout << "< formatter pattern error : " << i.name << " " << a.type
                                      << " " << a.pattern << " > " << std::endl;
                    ...
                    switch (a.type) {
                        ...
                        case AppenderType::Type::FileLogAppender:
                            ...
                            break;
                        case AppenderType::Type::RotatingFileLogAppender:
                            builder->BuildLogAppender<RotatingFileLogAppender>(
                                a.file, ll, lap, a.max_size, a.max_file, a.flush_rule);
                            break;
                        default:
                ...
                if (i.bufMgr.isValid()) {
                    BufferParams bufParams(i.bufMgr.type, i.bufMgr.size, i.bufMgr.threshold,
                                           i.bufMgr.linear_growth, i.bufMgr.swap_time);
