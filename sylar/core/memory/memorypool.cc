...
static sylar::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr
    g_memPool_config = sylar::Config::Lookup(
        "memory_pool", std::map<std::string, std::map<std::string, std::string>>(), "MemoryPool");

static sylar::ConfigVar<int>::ptr g_protectStackPageSize =
    sylar::Config::Lookup("protect_stack_pagesize", 1, "protectStackPageSize");

static Logger::ptr g_logger = SYLAR_LOG_NAME("system");

bool ProtectStack(void *ptr, std::size_t size)
{
    int pageSize = g_protectStackPageSize->getValue();
    if (!pageSize)
        return false;
    // x86 getpagesize()=4KB
    ...
    // 确保内存保护的起始地址严格对齐到4KB页面边界
    void *protect_page_addr = ((std::size_t)ptr & 0xfff)
                                  ? (void *)(((std::size_t)ptr & ~(std::size_t)0xfff) + 0x1000)
                                  : ptr;

    if (-1 == mprotect(protect_page_addr, getpagesize() * pageSize, PROT_NONE)) {
        SYLAR_LOG_FMT_ERROR(g_logger,
                            "origin_addr:%p, align_addr:%p, page_size:%d, protect_page:%u, protect "
                            "stack stack error: %s",
    ...
    if (splitNode != nullptr) {
        ...
        *reinterpret_cast<void **>(splitNode) = nullptr;
        ...
        if (returnNum > 0 && nextNode != nullptr) {
            CentralCache::GetInstance().returnRange(nextNode, returnNum * (index + 1) * ALIGNMENT,
                                                    index);
        }
    }
}
...
size_t ThreadCache::getBatchNum(size_t size)
{
    ...
    if (size <= 32)
        ...
        baseNum = 1; // 大于 1024的对象每次只从中心缓存取1个
...
void *CentralCache::fetchRange(size_t index, size_t batchNum, size_t &realBatchNum)
{
    ...
    while (locks_[index].test_and_set(std::memory_order::acquire)) {
