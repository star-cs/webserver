...
// Dns 配置项：支持从 YAML 读取域名/类型/连接池大小/健康检查路径/静态地址集合
struct DnsDefine {
    std::string domain;
    int type;
    int pool_size = 0;
...
// 配置监听器：dns.config 变更时，创建/更新对应的 Dns 实例
struct DnsIniter {
    DnsIniter()
    {
...
// 初始化地址（TYPE_ADDRESS）：解析字符串地址 -> Address 列表，并执行健康检查
void Dns::init()
{
...
// 健康检查（可选 HTTP 检查）；根据结果更新 valid，并维护连接池
bool Dns::AddressItem::checkValid(uint32_t timeout_ms)
{
    ...
    if (valid) {
        if (!check_path.empty()) {
            ...
            sylar::http::HttpConnection hc(sock_ptr, false);
            hc.sendRequest(req);
            auto rsp = hc.recvResponse();
            if (!rsp || (int)rsp->getStatus() != 200) {
                valid = false;
                SYLAR_LOG_ERROR(g_logger)
                    << "health_check fail"
                    << " rsp.status=" << (rsp ? (int)rsp->getStatus() : -1)
...
// 根据解析结果构建/复用 AddressItem，并执行健康检查
...
// 刷新：TYPE_DOMAIN 重新解析域名；TYPE_ADDRESS 刷新静态地址
void Dns::refresh()
{
...
// 获取服务地址：若未命中且 cache=true，会异步加入并返回直接解析的地址
...
// 触发一次刷新所有已注册的 Dns（受 m_refresh 保护）
