...
void IOManager::FdContext::triggerEvent(Event event)
{
     ...
     * 注册的IO事件是一次性的，如果想持续关注某个socket
     * fd的读写事件，那么每次触发事件后都要重新添加。~
     */
    ...
    : Scheduler(threads, use_caller, name)
{
    ...
    ev.events =
        EPOLLIN | EPOLLET; // 边沿触发  绑定pipe读端。 我们就可以对pipe写端写入，以tickle协程
...
{
    SYLAR_LOG_DEBUG(g_logger) << "addEvent called, fd=" << fd << ", event=" << (EPOLL_EVENTS)event
    ...
    SYLAR_ASSERT(!ev_ctx.scheduler && !ev_ctx.fiber && !ev_ctx.cb);
    ...
    if (cb) {
        ...
        SYLAR_ASSERT2(ev_ctx.fiber->getState() == Fiber::RUNNING,
                      "state=" << ev_ctx.fiber->getState());
    }
    return 0;
}
...
{
    SYLAR_LOG_DEBUG(g_logger) << "delEvent called, fd=" << fd << ", event=" << (EPOLL_EVENTS)event;
    FdContext *fd_ctx = nullptr;
    RWMutexType::ReadLock lock(m_mutex);
    if ((int)m_fdContexts.size() <= fd) {
...
{
    SYLAR_LOG_DEBUG(g_logger) << "cancelEvent called, fd=" << fd
 ...
 * 1. 有没有新的调度任务，对应 Scheduler::schedule();
 * 如果有新的调度任务，那应该立即退出idle状态，并执行相应的任务；
...
void IOManager::idle()
{
    ...
    std::shared_ptr<epoll_event> shared_events(events, [](epoll_event *ev) { delete[] ev; });
