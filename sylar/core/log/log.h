...
#define SYLAR_LOG_LEVEL(logger, level)                                                             \
    ...
    sylar::LogEventWrap(logger,                                                                    \
                        sylar::LogEvent::ptr(new sylar::LogEvent(                                  \
                            std::string(__FILE__), __LINE__, 0, sylar::GetThreadId(),              \
                            sylar::Thread::GetName(), sylar::GetFiberId(), time(0), level)))       \
        .getSS()
...
#define SYLAR_LOG_FMT_LEVEL(logger, level, fmt, ...)                                               \
    ...
    sylar::LogEventWrap(logger,                                                                    \
                        sylar::LogEvent::ptr(new sylar::LogEvent(                                  \
                            ...
                            sylar::GetFiberId(), time(0), level)))                                 \
        .getEvent()                                                                                \
        ->format(fmt, __VA_ARGS__)

#define SYLAR_LOG_FMT_DEBUG(logger, format, ...)                                                   \
    SYLAR_LOG_FMT_LEVEL(logger, sylar::LogLevel::DEBUG, format, __VA_ARGS__)
#define SYLAR_LOG_FMT_INFO(logger, format, ...)                                                    \
    SYLAR_LOG_FMT_LEVEL(logger, sylar::LogLevel::INFO, format, __VA_ARGS__)
#define SYLAR_LOG_FMT_WARN(logger, format, ...)                                                    \
    SYLAR_LOG_FMT_LEVEL(logger, sylar::LogLevel::WARN, format, __VA_ARGS__)
#define SYLAR_LOG_FMT_ERROR(logger, format, ...)                                                   \
    SYLAR_LOG_FMT_LEVEL(logger, sylar::LogLevel::ERROR, format, __VA_ARGS__)
#define SYLAR_LOG_FMT_FATAL(logger, format, ...)                                                   \
    SYLAR_LOG_FMT_LEVEL(logger, sylar::LogLevel::FATAL, format, __VA_ARGS__)
...
public:
    ...
    LogEvent(std::string file, int32_t line, uint32_t elapse, uint32_t threadId,
             const std::string &threadName, uint32_t fiberId, uint64_t time, LogLevel::Level level);
    ...
    static LogEvent::ptr deserialize(Buffer &buffer)
    {
        ...
        // 6. 构建日志事件对象
        auto event = std::make_shared<LogEvent>(std::move(file), meta.line, meta.elapse,
                                                meta.threadId, std::move(thread_name), meta.fiberId,
                                                meta.timestamp, meta.level);
...
public:
    ...
    RotatingFileLogAppender(const std::string &filename, LogLevel::Level level,
                            LogFormatter::ptr formatter, size_t max_size,
...
public:
    ...
    Logger(const std::string name, LogLevel::Level level, std::vector<LogAppender::ptr> &appenders,
           const BufferParams &bufParams)
        : m_name(name), m_level(level), m_appenders(appenders.begin(), appenders.end())
    {
        ...
        while (true) { // 解析 buffer
            ...
            if (event) {
                ...
                if (buffer->readableSize() == 0) { // 读完了
                    ...
                    // 处理失败但数据未读完（说明发生严重错误）
                    std::cout << "Log deserialization error, remaining data: "
    ...
    std::vector<LogAppender::ptr> getAppenders() const;

    void clearAppender();

    void setBufferManager(BufferManager::ptr bufManager) { m_bufMgr = bufManager; }
    BufferManager::ptr getBufferManager() const { return m_bufMgr; }
...
public:
    ...
    // 默认构造root，UNKNOW
    LoggerBuilder(const std::string &name = "root", LogLevel::Level level = LogLevel::Level::UNKNOW)
    ...
    {
        EnsureDefaults();
        return std::make_shared<Logger>(m_logger_name, m_level, m_appenders, m_bufParams);
    }
...
private:
    void EnsureDefaults()
    {
        if (m_appenders.empty()) {
            BuildLogAppender<StdoutLogAppender>(LogLevel::UNKNOW, std::make_shared<LogFormatter>());
        }
    }
