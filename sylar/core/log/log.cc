...
{
public:
    DataTimeFormatterItem(const std::string &format = "%Y-%m-%d %H:%M:%S") : m_format(format)
    {
        if (m_format.empty()) {
...
public:
    ...
    void format(std::ostream &os, Logger::ptr logger, LogEvent::ptr event) override { os << "\t"; }
};

// LogEvent

LogEvent::LogEvent(std::string file, int32_t line, uint32_t elapse, uint32_t threadId,
                   const std::string &threadName, uint32_t fiberId, uint64_t time,
                   LogLevel::Level level)
    : m_file(file), m_line(line), m_elapse(elapse), m_threadId(threadId), m_threadName(threadName),
...
std::vector<LogAppender::ptr> Logger::getAppenders() const
{
    return m_appenders;
}
...
FileLogAppender::FileLogAppender(const std::string &filename, LogLevel::Level level,
                                 LogFormatter::ptr formatter, FlushRule::Rule flush_rule)
    : LogAppender(level, formatter), m_filename(filename), m_flushRule(flush_rule)
{
    ...
    if (!m_fs) {
        std::cerr << "Failed to open log file: " << filename << " Error: " << strerror(errno)
                  << std::endl;
    }
}
...
// 给的filename应该是一个文件夹
RotatingFileLogAppender::RotatingFileLogAppender(const std::string &filename, LogLevel::Level level,
                                                 LogFormatter::ptr formatter, size_t max_size,
                                                 size_t max_file, FlushRule::Rule flush_rule)
    : LogAppender(level, formatter), m_filename(filename), m_maxSize(max_size), m_maxFile(max_file),
      m_flushRule(flush_rule), m_buffer(max_size) // 预分配数据大小
{
    assert(m_flushRule != FlushRule::UNKNOW);
    assert(m_maxSize > 0);
    FSUtil::Mkdir(FSUtil::Dirname(m_filename));
        ...
        if (m_curFile != NULL) {
            ...
            if (m_maxFile == 0) {
                ...
                if (!m_fileNames[m_curFileIndex].empty()) { // 说明 循环到 已有的文件了。
                    ...
                    if (rename(m_fileNames[m_curFileIndex].c_str(), newfilename.c_str())
                        != 0) { // 文件 改新名字
                    ...
                    fseek(
                        m_curFile, 0,
                        SEEK_SET); // 从头 覆盖，不考虑 日志文件名了。默认最后一个文件可能会存在过往的日志信息。
                    m_curFilePos = 0;
                    return;
                }
            }
...
void LogFormatter::init()
{
    ...
    for (size_t i = 0; i < m_pattern.size(); ++i) {
        ...
        while (pos < m_pattern.size()) {
            // 状态0：寻找格式项结束或{
            if (fmt_status == 0 && !isalpha(m_pattern[pos]) && // 非字母字符作为分隔
                m_pattern[pos] != '{' && m_pattern[pos] != '}') {
    ...
    // 格式项映射表（关键说明）
    static std::map<std::string, std::function<FormatterItem::ptr(const std::string &)>>
        s_format_items = {
    // 格式说明：
    ...
    for (auto &i : vec) {
        if (std::get<2>(i) == 0) {
            ...
            if (it == s_format_items.end()) {
                // 无效格式项处理
                m_items.push_back(std::make_shared<StringFormatterItem>("<<error_format %"
                                                                        + std::get<0>(i) + ">>"));
                error = true;
            } else {
                // 创建对应的格式化项（如时间格式化项）
...
LoggerManager::LoggerManager()
{
    ...
    builder->BuildLogAppender<StdoutLogAppender>(LogLevel::UNKNOW,
...
void LoggerManager::addLogger(const Logger::ptr &logger)
{
    ...
    if (it != m_loggers.end()) { // 不能完全覆盖, static logger 以及指向了 old_logger
        auto old_logger = it->second;
        old_logger->setLevel(logger->getLevel());
        old_logger->clearAppender();
        for (auto &i : logger->getAppenders()) {
            old_logger->addAppender(i);
        }
        old_logger->setBufferManager(logger->getBufferManager());
    } else {
