...
public:
    ...
    static void warmup() 
    {
        ...
        {
            sylar::threadCache::GetInstance()->deallocate(it.first, it.second);  // 使用实际分配的大小进行释放
        }
    ...
    static void testMultiThreaded() 
    {
        ...
        auto threadFunc = [](bool useMemPool) 
        {
            ...
            {
                size_t size = dis(gen);
                void* ptr = useMemPool ? sylar::threadCache::GetInstance()->allocate(size) 
                                     : new char[size];
                ...
                {  // 75%的概率释放
                    ...
                    if (useMemPool) {
                        sylar::threadCache::GetInstance()->deallocate(ptrs[index].first, ptrs[index].second);
                    } else {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) 
            {
                ...
                if (i % 100 == 0 && !ptrs.empty()) 
                {
                    ...
                    {
                        sylar::threadCache::GetInstance()->deallocate(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
