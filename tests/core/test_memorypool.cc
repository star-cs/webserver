    ...
    static void warmup()
    {
        ...
        for (int i = 0; i < 1000; ++i) {
            for (size_t size : {32, 64, 128, 256, 512}) {
                void *p = sylar::ThreadCache::GetInstance()->allocate(size);
                warmupPtrs.emplace_back(p, size); // 存储指针和对应的大小
            }
        }

        // 释放预热内存
        for (const auto &it : warmupPtrs) {
            sylar::ThreadCache::GetInstance()->deallocate(it.first,
                                                          it.second); // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation()
    {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) {
                ptrs.push_back(sylar::ThreadCache::GetInstance()->allocate(SMALL_SIZE));

                // 模拟真实使用：部分立即释放
                if (i % 4 == 0) {
                    sylar::ThreadCache::GetInstance()->deallocate(ptrs.back(), SMALL_SIZE);
                    ptrs.pop_back();
                }
            }

            for (void *ptr : ptrs) {
                sylar::ThreadCache::GetInstance()->deallocate(ptr, SMALL_SIZE);
            }

            std::cout << "Memory Pool: " << std::fixed << std::setprecision(3) << t.elapsed()
                      << " ms" << std::endl;
        }
        ...
        // 测试new/delete
        {
            ...
            std::cout << "New/Delete: " << std::fixed << std::setprecision(3) << t.elapsed()
                      << " ms" << std::endl;
        }
    }
    ...
    static void testMultiThreaded()
    {
        ...
        auto threadFunc = [](bool useMemPool) {
            ...
            for (size_t i = 0; i < ALLOCS_PER_THREAD; ++i) {
                size_t size = dis(gen);
                void *ptr =
                    useMemPool ? sylar::ThreadCache::GetInstance()->allocate(size) : new char[size];
                ...
                if (rand() % 100 < 75) { // 75%的概率释放
                    ...
                    if (useMemPool) {
                        sylar::ThreadCache::GetInstance()->deallocate(ptrs[index].first,
                                                                      ptrs[index].second);
                    } else {
            ...
            for (const auto &it : ptrs) {
                if (useMemPool) {
                    sylar::ThreadCache::GetInstance()->deallocate(it.first, it.second);
                } else {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) {
                ...
                if (i % 100 == 0 && !ptrs.empty()) {
                    ...
                    for (size_t j = 0; j < releaseCount; ++j) {
                        sylar::ThreadCache::GetInstance()->deallocate(ptrs.back().first,
                                                                      ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
